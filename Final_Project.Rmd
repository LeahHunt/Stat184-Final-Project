...
The most obvious factor that may come to mind is that he also got married between these two seasons. While this factor will not be considered in this analysis, in other analyses, marriage has not been shown to correlate with better statistics (see: ![http://ftp.iza.org/dp5695.pdf]).
...
Now that we have merged teh tables,let's see how the two distrbutions compare. In the below plots, we see MCutchen's performance (one for OBP and one for BA) in blue and the distribution of the average of MLB players in red. The solid black lines show the time he joined and left the Pirates and the dotted line shows the 2014 season, which is where we would expect the peak to be. Note that in defining the maximum value for McCutchen, only statistics in his time with the Pirates were considered.
```{r}
merged_player_data_with_McCutchen %>% mutate(difference = BApercent_M-BApercent)%>%filter(age>17 & age<33) %>%ggplot()  +
  geom_point(aes(x=age, y=BApercent), color='red') + geom_point(aes(x=age, y=BApercent_M),color='blue')+ stat_smooth(aes(x=age, y=BApercent), color='red', se=FALSE)+ stat_smooth(aes(x=age, y=BApercent_M), se=FALSE) + geom_vline(xintercept=22)+ geom_vline(xintercept=31)+ geom_vline(xintercept=27, linetype="dotted")+ ylim(.7,1)
  theme(axis.text.x = element_text(angle = 30, hjust=1))
```
```{r}
merged_player_data_with_McCutchen %>% mutate(difference = OBPpercent_M-OBPpercent)%>%filter(age>17 & age<33) %>%ggplot()  +
  geom_point(aes(x=age, y=OBPpercent), color='red') + geom_point(aes(x=age, y=OBPpercent_M), color='blue')+ stat_smooth(aes(x=age, y=OBPpercent), color='red', se=FALSE)+ stat_smooth(aes(x=age, y=OBPpercent_M), se=FALSE)+ geom_vline(xintercept=22)+ geom_vline(xintercept=31)+ geom_vline(xintercept=27, linetype="dotted")
  theme(axis.text.x = element_text(angle = 30, hjust=1))
```
...
```{r}
merged_player_data%>%filter(AB>20)%>%filter(OBP==maxobp)%>%ggplot()+geom_histogram(aes(age), fill='cyan', alpha=.5)+ geom_vline(xintercept=27, linetype="dotted")
```

```{r}
merged_player_data%>%filter(AB>20)%>%filter(BA==maxba)%>%ggplot()+geom_histogram(aes(age), fill='cyan', alpha=.5)+ geom_vline(xintercept=27, linetype="dotted")
```
